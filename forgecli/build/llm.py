...
async def llm_call(context: BuildContext) -> BuildContext:
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            # Apply patch generation/recovery layer
            from forgecli.plugins import Intent
            if intent in (None, Intent.BUILD, "build"):
                from dataclasses import replace
                from forgecli.build.patch_generator import PatchGenerator
                patched_content = PatchGenerator.ensure_patch(
                    response.message.content,
                    context.root,
                    context.prompt
                )
                new_msg = replace(response.message, content=patched_content)
                response = response.model_copy(update={"message": new_msg})
